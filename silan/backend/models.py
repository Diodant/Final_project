...
class Machine(models.Model):
    ...
    factory_number_machine = models.CharField(max_length=128, unique=True,
                                              verbose_name='Заводской номер машины')  # Зав. № машины
    models_machine = models.ForeignKey('ModelsMachine', on_delete=models.DO_NOTHING,
                                       verbose_name='Модель техники')  # Модель техники
    models_engine = models.ForeignKey('ModelsEngine', on_delete=models.DO_NOTHING,
                                      verbose_name='Модель двигателя')  # Модель двигателя
    factory_number_engine = models.CharField(max_length=128, verbose_name='Зав. № двигателя')  # Зав. № двигателя
    model_transmission = models.ForeignKey('ModelsTransmission', on_delete=models.DO_NOTHING,
                                           verbose_name='Модель трансмиссии')  # Модель трансмиссии
    factory_number_transmission = models.CharField(max_length=128,
                                                   verbose_name='Зав. № трансмиссии')  # Зав. № трансмиссии
    models_drive_axle = models.ForeignKey('ModelsDriveAxle', on_delete=models.DO_NOTHING,
                                          verbose_name='Модель ведущего моста')  # Модель ведущего моста
    factory_number_drive_axle = models.CharField(max_length=128,
                                                 verbose_name='Зав. № ведущего моста')  # Зав. № ведущего моста
    models_steering_bridge = models.ForeignKey('ModelsSteeringBridge',
                                               on_delete=models.DO_NOTHING,
                                               verbose_name='Модель управляемого моста')  # Модель управляемого моста
    factory_number_steering_bridge = models.CharField(max_length=128,
                                                      verbose_name='Зав. № управляемого моста')  # Зав. № управляемого моста
    supply_contract = models.CharField(max_length=128,
                                       verbose_name='Договор поставки №, дата')  # Договор поставки №, дата
    ...
    consumer = models.CharField(max_length=128,
                                verbose_name='Грузополучатель (конечный потребитель)')  # Грузополучатель (конечный потребитель)
    delivery_address = models.CharField(max_length=128,
                                        verbose_name='Адрес поставки (эксплуатации)')  # Адрес поставки (эксплуатации)
    equipment = models.CharField(max_length=512, verbose_name='Комплектация (доп. опции)')  # Комплектация (доп. опции)
    client = models.ForeignKey(settings.AUTH_USER_MODEL, related_name='client', on_delete=models.DO_NOTHING,
                               verbose_name='Клиент')  # Клиент
...
class Maintenance(models.Model):
    ...
    type_maintenance = models.ForeignKey('TypeMaintenance', on_delete=models.DO_NOTHING,
                                         verbose_name='Вид ТО')  # Вид ТО
...
class Claims(models.Model):
    ...
    operating_time = models.PositiveIntegerField(verbose_name='Наработка, м/час')  # Наработка, м/час
    failure_node = models.ForeignKey('FailureNode', on_delete=models.DO_NOTHING,
                                     verbose_name='Узел отказа')  # Узел отказа
    description_failure = models.CharField(max_length=128, verbose_name='Описание отказа')  # Описание отказа
    recovery_method = models.ForeignKey('RecoveryMethod', on_delete=models.DO_NOTHING,
                                        verbose_name='Способ восстановления')  # Способ восстановления
    used_spare_parts = models.CharField(max_length=512,
                                        verbose_name='Используемые запасные части')  # Используемые запасные части
    ...
    machine_downtime = models.PositiveIntegerField(null=True,
                                                   verbose_name='Время простоя техники')  # Время простоя техники
